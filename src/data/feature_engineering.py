...
class MaintenanceFeatureEngineer:
    ...
    def create_rolling_features(
        self, df: pd.DataFrame, windows: List[int] = [3, 24]
    ) -> pd.DataFrame:
        """
        ...
        for window in windows:
            ...
            for sensor in self.sensor_columns:
                ...
                df_features[f"{sensor}_{window}h_mean"] = rolling_group.mean().reset_index(
                    0, drop=True
                )
                df_features[f"{sensor}_{window}h_std"] = rolling_group.std().reset_index(
                    0, drop=True
                )
                df_features[f"{sensor}_{window}h_max"] = rolling_group.max().reset_index(
                    0, drop=True
                )
                df_features[f"{sensor}_{window}h_min"] = rolling_group.min().reset_index(
                    0, drop=True
                )
        ...
        feature_count = len(
            [col for col in df_features.columns if any(f"{h}h_" in col for h in windows)]
        )
    ...
    def create_failure_labels(
        self, telemetry_df: pd.DataFrame, failures_df: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        print(
            f"Creating failure labels with {self.prediction_window_days}-day prediction window..."
        )
        ...
        for _, failure in failures_df.iterrows():
            ...
            mask = (
                (df_labeled["machineID"] == machine_id)
    ...
    def process_features(
        self, telemetry_df: pd.DataFrame, failures_df: pd.DataFrame, windows: List[int] = [3, 24]
...
def create_maintenance_features(
    telemetry_df: pd.DataFrame, failures_df: pd.DataFrame, prediction_window_days: int = 3
) -> Tuple[pd.DataFrame, List[str]]:
    """
...
if __name__ == "__main__":
    ...
    print(f"  Feature engineering test completed:")
